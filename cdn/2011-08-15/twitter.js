    ...
    window.getTwitters = function (target, id, count, options) {
        ...
        ready((function(options, guid) {
            return function () {
                ...
                var ssl = ("https:" == document.location.protocol) ? "https://" : "http://";
                var url = ssl+'api.twitter.com/1/statuses/' + (options.withFriends ? 'friends_timeline' : 'user_timeline') + '/' + id + '.json?callback=twitterCallback' + guid + '&count=20&cb=' + Math.random();
